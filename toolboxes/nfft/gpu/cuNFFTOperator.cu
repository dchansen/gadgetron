...
namespace Gadgetron {
    ...
    cuNFFTOperator<REAL, D>::cuNFFTOperator(ConvolutionType convolution) : linearOperator<cuNDArray < complext < REAL> > >() {
    plan_ = make_cuNFFT_plan<REAL,D>(convolution);
}
...
template<class REAL, unsigned int D>
void
cuNFFTOperator<REAL, D>::setup(typename uint64d<D>::Type matrix_size, typename uint64d<D>::Type matrix_size_os,
                               REAL W) {
...
template
class EXPORTGPUNFFT cuNFFTOperator<float, 1>;

template
class EXPORTGPUNFFT cuNFFTOperator<float, 2>;

template
class EXPORTGPUNFFT cuNFFTOperator<float, 3>;

template
class EXPORTGPUNFFT cuNFFTOperator<float, 4>;

template
class EXPORTGPUNFFT cuNFFTOperator<double, 1>;

template
class EXPORTGPUNFFT cuNFFTOperator<double, 2>;

template
class EXPORTGPUNFFT cuNFFTOperator<double, 3>;

template
class EXPORTGPUNFFT cuNFFTOperator<double, 4>;
}
